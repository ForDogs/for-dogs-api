...
(
    ...
    `provider`     ENUM ('GOOGLE', 'KAKAO', 'LOCAL') NOT NULL DEFAULT 'LOCAL' COMMENT '공급자 (GOOGLE: 구글, KAKAO:카카오, LOCAL: 일반)',
...
(
    ...
    `cancellation_id`          VARCHAR(50)  NULL COMMENT '결제 취소 ID',
    `cancellation_at`          INT          NULL COMMENT '결제 취소 시각',
    `cancellation_amount`      INT          NULL COMMENT '결제건의 누적 취소 금액',
    `cancellation_receipt_url` VARCHAR(200) NULL COMMENT '취소 매출전표 확인 URL',
    `cancellation_reason`      VARCHAR(255) NULL COMMENT '결제 취소 사유',
...
CREATE TABLE `for_dog_db`.`chat_rooms`
(
    `id`         BINARY(16)   NOT NULL COMMENT '채팅방 ID',
    `name`       VARCHAR(100) NOT NULL COMMENT '채팅방 이름',
    `deleted`    TINYINT(1)   NOT NULL DEFAULT 0 COMMENT '채팅방 삭제 여부',
    `created_at` DATETIME     NOT NULL DEFAULT NOW() COMMENT '최초 생성 일시',
    `updated_at` DATETIME     NOT NULL DEFAULT NOW() ON UPDATE NOW() COMMENT '최종 수정 일시',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4 COMMENT ='채팅방 정보';

CREATE TABLE `for_dog_db`.`chat_room_users`
(
    `id`           BINARY(16) NOT NULL COMMENT '채팅방 사용자 고유 ID',
    `chat_room_id` BINARY(16) NOT NULL COMMENT '채팅방 ID',
    `user_id`      BINARY(16) NOT NULL COMMENT '사용자 ID',
    `created_at`   DATETIME   NOT NULL DEFAULT NOW() COMMENT '최초 생성 일시',
    `updated_at`   DATETIME   NOT NULL DEFAULT NOW() ON UPDATE NOW() COMMENT '최종 수정 일시',
    PRIMARY KEY (`id`),
    CONSTRAINT `fk_chat_room_users_chat_room_id` FOREIGN KEY (`chat_room_id`) REFERENCES `for_dog_db`.`chat_rooms` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT `fk_chat_room_users_user_id` FOREIGN KEY (`user_id`) REFERENCES `for_dog_db`.`user` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4 COMMENT ='채팅방 사용자 정보';

CREATE TABLE `for_dog_db`.`chat_messages`
(
    `id`           BINARY(16) NOT NULL COMMENT '메시지 ID',
    `chat_room_id` BINARY(16) NOT NULL COMMENT '채팅방 ID',
    `user_id`      BINARY(16) NOT NULL COMMENT '사용자 ID',
    `content`      TEXT       NOT NULL COMMENT '메시지 내용',
    `deleted`      TINYINT(1) NOT NULL DEFAULT 0 COMMENT '메시지 삭제 여부',
    `created_at`   DATETIME   NOT NULL DEFAULT NOW() COMMENT '최초 생성 일시',
    `updated_at`   DATETIME   NOT NULL DEFAULT NOW() ON UPDATE NOW() COMMENT '최종 수정 일시',
    PRIMARY KEY (`id`),
    CONSTRAINT `fk_chat_messages_chat_room_id` FOREIGN KEY (`chat_room_id`) REFERENCES `for_dog_db`.`chat_rooms` (`id`) ON DELETE SET NULL,
    CONSTRAINT `fk_chat_messages_sender_id` FOREIGN KEY (`user_id`) REFERENCES `for_dog_db`.`user` (`id`) ON DELETE CASCADE
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4 COMMENT ='채팅 메시지 정보';
